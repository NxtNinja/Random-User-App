...
const UserCard = ({info}: {info: Result}) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>{info.name.title} {info.name.first} {info.name.last}</Text>
      <Text style={styles.email}>{info.email}</Text>
      <Text style={styles.address}>{info.location.street.number}, {info.location.street.name}, {info.location.city} {info.location.state}, {info.location.country}-{info.location.postcode}</Text>
      <TouchableOpacity style={styles.button} onPress={() => refetch(info.gender)}>
